...
const router = (entry, server) => {
    ...
    server.post(`${entry}/current/pic`, async ({ files, uid }) => await handleRes(updateProfilePicture, uid, files[0])())
    ...
    server.post(`${entry}/current/profile`, async ({ body: { name, lastName }, uid }) => await handleRes(updateUser, uid, name, lastName)())
}
