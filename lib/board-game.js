...
export const getRendered = game =>
  Object.keys(game.objects)
    .reduce((acc, key) => acc.concat(game.objects[key]), [])
    .filter(obj =>
...
export const isOnBoard = (game, position) => (
  position.x >= 0
  && position.x < game.ext.gameSize.x
  && position.y >= 0
  && position.y < game.ext.gameSize.y
)